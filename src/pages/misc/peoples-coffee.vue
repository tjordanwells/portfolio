<template>
    <v-container class="py-16">
        ...
        <v-row class="py-16 text-h3 hel-neue-light text-center">
            <v-col cols="12">
                <p>{{ pc?.title }}</p>
                <p class="text-h5 py-4">{{ pc?.role }}</p>
            </v-col>
        </v-row>
        ...
        <v-row>
            <v-col cols="12">
                <v-row>
                    ...
                    <v-col>
                        <v-carousel height="700" hide-delimiters :show-arrows="false" v-model="slide">
...
export default {
    setup() {
        const pc = projects.projects[0]
        const desc_array = pc.desc.split('>')
        const state = reactive({
            slide: 0
        })

        const keys_array = Object.keys(pc.images)
        const length = keys_array.length - 1

        return { pc, desc_array, length, ...toRefs(state) }
    }
}
</script>
